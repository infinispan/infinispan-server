...
class EndpointSubsystemParser implements XMLStreamConstants, XMLElementReader<List<ModelNode>>,
   ...
   private void readConnector(final XMLExtendedStreamReader reader, final ModelNode operation) {
      ...
      if (socketBinding == null) {
         ParseUtils
                  .missingRequired(reader, Collections.singleton(DataGridConstants.SOCKET_BINDING));
      }
   ...
   private void readTopologyStateTransfer(final XMLExtendedStreamReader reader,
      ...
      for (int i = 0; i < attrCnt; i++) {
         ...
         if (DataGridConstants.LOCK_TIMEOUT.equals(attrName)) {
            topologyStateTransfer.get(DataGridConstants.LOCK_TIMEOUT).set(
                     Integer.parseInt(attrValue));
         } else if (DataGridConstants.REPLICATION_TIMEOUT.equals(attrName)) {
            topologyStateTransfer.get(DataGridConstants.REPLICATION_TIMEOUT).set(
                     Integer.parseInt(attrValue));
         } else if (DataGridConstants.UPDATE_TIMEOUT.equals(attrName)) {
            topologyStateTransfer.get(DataGridConstants.UPDATE_TIMEOUT).set(
                     Integer.parseInt(attrValue));
            ...
            topologyStateTransfer.get(DataGridConstants.EXTERNAL_PORT).set(
                     Integer.parseInt(attrValue));
         } else if (DataGridConstants.LAZY_RETRIEVAL.equals(attrName)) {
            topologyStateTransfer.get(DataGridConstants.LAZY_RETRIEVAL).set(
   ...
   private void writeTopologyStateTransfer(final XMLExtendedStreamWriter writer,
      ...
      if (node.hasDefined(DataGridConstants.TOPOLOGY_STATE_TRANSFER)) {
         ...
         if (topologyStateTransfer.hasDefined(DataGridConstants.LOCK_TIMEOUT)) {
            writer.writeAttribute(DataGridConstants.LOCK_TIMEOUT,
                     topologyStateTransfer.get(DataGridConstants.LOCK_TIMEOUT).asString());
         }
         if (topologyStateTransfer.hasDefined(DataGridConstants.REPLICATION_TIMEOUT)) {
            writer.writeAttribute(DataGridConstants.REPLICATION_TIMEOUT,
                     topologyStateTransfer.get(DataGridConstants.REPLICATION_TIMEOUT).asString());
         }
         if (topologyStateTransfer.hasDefined(DataGridConstants.UPDATE_TIMEOUT)) {
            writer.writeAttribute(DataGridConstants.UPDATE_TIMEOUT,
                     topologyStateTransfer.get(DataGridConstants.UPDATE_TIMEOUT).asString());
         }
         if (topologyStateTransfer.hasDefined(DataGridConstants.EXTERNAL_HOST)) {
            writer.writeAttribute(DataGridConstants.EXTERNAL_HOST,
                     topologyStateTransfer.get(DataGridConstants.EXTERNAL_HOST).asString());
         }
         if (topologyStateTransfer.hasDefined(DataGridConstants.EXTERNAL_PORT)) {
            writer.writeAttribute(DataGridConstants.EXTERNAL_PORT,
                     topologyStateTransfer.get(DataGridConstants.EXTERNAL_PORT).asString());
         }
         if (topologyStateTransfer.hasDefined(DataGridConstants.LAZY_RETRIEVAL)) {
            writer.writeAttribute(DataGridConstants.LAZY_RETRIEVAL,
                     topologyStateTransfer.get(DataGridConstants.LAZY_RETRIEVAL).asString());
         }
      }
   }
}
